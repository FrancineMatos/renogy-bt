...
class DataLogger:
    ...
    def create_mqtt(self, json_data):
        ...
        device_info = json_data.get('device_info')
        if not device_info:
            logging.warning("MQTT: device_info not found, skipping publish")
            return
        ...
        for data in device_info:
            ...
            icon = json.dumps(json_data['device_info'][data]['icon'])
            icon = icon.replace("\"","")
            ...
            if device_class == "temperature":
                arrJSON['device_class'] = "temperature"
                arrJSON['unit_of_measurement'] = "°C"                
    ...
    def log_mqtt(self, json_data):
        ...
        for data in json_data['device_info']:
            data_value = json.dumps(json_data['device_info'][data]['value'])
            data_value = data_value.replace("\"","")

            publish.single(
                self.config['mqtt']['topic']+"/"+data+"/status", payload=data_value,
                hostname=self.config['mqtt']['server'], port=self.config['mqtt'].getint('port'),
                auth=auth, client_id="renogy-bt"
            )
